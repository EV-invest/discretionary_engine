...
fn make_id(s: &str) -> String {
	...
	let collapsed: String = out.split('_').filter(|s| !s.is_empty()).collect::<Vec<_>>().join("_");
	collapsed
}
...
fn main() -> ExitCode {
	let root = std::env::var("EX_ROOT").unwrap_or_else(|_| {
		let output = std::process::Command::new("git").args(["rev-parse", "--show-toplevel"]).output().expect("git not found");
		String::from_utf8(output.stdout).unwrap().trim().to_string()
	});
	let root = Path::new(&root);
	let in_path = root.join("docs/arch.excalidraw");
	let out_path = root.join("docs/arch.md");
	...
	// Sort by x position of associated rect
	nodes.sort_by(|a, b| {
